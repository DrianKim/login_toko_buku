<div id="editHargaModal{{ $buku->id }}" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-white/20 backdrop-blur-sm backdrop-saturate-150 transition-all">
    <div class="bg-white rounded-xl shadow-lg w-full max-w-md p-6 relative">
        <h2 class="text-xl font-semibold text-gray-800 mb-4">Edit Harga Buku</h2>

        <form action="{{ route('admin.detail-buku.update-harga', $buku->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="flex justify-end gap-3 mt-6">
                <button type="button"
                    ...
                    onclick="document.getElementById('editHargaModal{{ $buku->id }}').classList.add('hidden')">
                    Batal
                </button>
                <button type="submit"
